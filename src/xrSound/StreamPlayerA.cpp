...
CStreamPlayerA::CStreamPlayerA(ALuint sampleRate, ALenum format, ALCcontext* context) : m_sampleRate(sampleRate), m_format(format), m_pContext(context), m_source(0)
{
...
void CStreamPlayerA::PushToPlay(const void* data, int count)
{
	...
	if (m_source == 0)
	{
		Msg("[OpenAL][Voice] Not Initilized Sound Source? (SoundPlayer OFF)");
		return;
	}

	if (!alIsSource(m_source))
	{
		Msg("[OpenAL][Voice] alIsSource? (SoundPlayer OFF)");
		return;
	}
